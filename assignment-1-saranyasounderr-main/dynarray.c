...
struct dynarray* dynarray_create() {
  struct dynarray *newArray = malloc(sizeof(struct dynarray)); //allocate memory for a struct
  newArray->capacity = 2;
  newArray->size = 0;
  newArray->data = malloc(newArray->capacity *(sizeof(void*))); //allocate memory for an array
  return newArray;
}
...
//note: Have to free everytime malloc is called.
...
void dynarray_insert(struct dynarray* da, void* val) {
  if(da->capacity <= da->size){
    //printf("increasing capacity"); //checking
    da->capacity*=2;
    da->data = realloc(da->data, da->capacity * sizeof(void*)); //reallocating memory
  }
  da->data[da->size] = val;
  da->size ++;
  return;
...
void dynarray_remove(struct dynarray* da, int idx) {
  da->size --;
  //printf("removing"); //checking
  for (int i = idx; i < da->size; i++){
    da->data[i] = da->data[i + 1]; //moving the data by a spot(essentially removing the selected date)
  }
  return;
}
...
void dynarray_set(struct dynarray* da, int idx, void* val) {
  //printf("assigning"); //checking
  da->data[idx] = val; //setting the value at the index
  return;
}
