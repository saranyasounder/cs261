...
{
  struct link *curLink = list->head;
  while (curLink != NULL)
  {
    struct link *nextLink = curLink->next;
    free(curLink); //freeing curlink(no mem leaks)
    curLink = nextLink;
  }
...
{
  struct link *newLink = malloc(sizeof(struct link)); //allocating new memory for a link
  newLink->val = val;
...
{
  struct link *curLink = list->head;
  if (list->head == NULL) //case 1: no entries
  { 
    return;
  }
  else if (list->head->next == NULL)//case 2: only one entry
  {
    if (cmp(curLink->val, val) == 0)
    {
      free(list->head);
      list->head = NULL;
      return;
  ...
  else if (cmp(list->head->val, val) == 0) //case 3: the head is the matching value
  {
    list->head = curLink->next; // the next link is the head
    free(curLink);
    return;
  }
  // Go through the list to find the target value.
  while (curLink->next != NULL) //case 4: the rest of the scenarios
  {
    if (cmp(curLink->next->val, val) == 0)
    {
      struct link *removeLink = curLink->next;
      curLink->next = removeLink->next; //assigning the new links
      free(removeLink);
      return;
    }
...
int list_position(struct list *list, void *val, int (*cmp)(void *a, void *b))
{
  ...
  while (curLink != NULL)
  {
    if (cmp(curLink->val, val) == 0)
    {
      return position; //returning the position number
    }
    position++; //increasing the position number
    curLink = curLink->next; //check the next link
  }
  return -1;
}
...
void list_reverse(struct list *list)
{
  ...
  while (curLink != NULL)
  {
    nextLink = curLink->next; //(keeping track of the next link)
    curLink->next = prevLink; //reversing the position
   // updating the links
    prevLink = curLink; 
    curLink = nextLink;
  }
